...
func StorePlateRecognitionReport(plate string, report UserReport) error {
	...
	rttmas_redis.GetRedis().SAdd(context.Background(), fmt.Sprintf("plate_recognition_report:%d", report.ReportTime), utils.Jsonalize(report))
	rttmas_redis.GetRedis().GeoAdd(context.Background(), fmt.Sprintf("plate_recognition_report:%d", report.ReportTime), &redis.GeoLocation{
		Name:      plate,
		Latitude:  report.Latitude,
		Longitude: report.Longitude,
	})
	rttmas_redis.GetRedis().Expire(context.Background(), fmt.Sprintf("plate_recognition_report:%d", report.ReportTime), 10*time.Second)
	rttmas_redis.GetRedis().GeoAdd(context.Background(), fmt.Sprintf("plate_locations:%s", plate), &redis.GeoLocation{
		Name:      fmt.Sprintf("%d", report.ReportTime),
		Latitude:  report.Latitude,
		Longitude: report.Longitude,
	})
	rttmas_redis.GetRedis().Expire(context.Background(), fmt.Sprintf("plate_locations:%s", plate), 10*time.Second)

	exists, err := rttmas_redis.GetRedis().Exists(context.Background(), fmt.Sprintf("basic_info:%s", plate)).Result()
	if err != nil {
		// handle error
	}
	if exists == 0 {
		initialData := `{}`
		_, err = rttmas_redis.GetRedis().JSONSet(context.Background(), fmt.Sprintf("basic_info:%s", plate), "$", initialData).Result()
		rttmas_redis.GetRedis().Expire(context.Background(), fmt.Sprintf("basic_info:%s", plate), 10*time.Second)
	...
	rttmas_redis.GetRedis().JSONSet(context.Background(), fmt.Sprintf("basic_info:%s", plate), "$.latest_report_time", fmt.Sprintf("%d", report.ReportTime))
...
func StoreUserLocationReport(ulr UserReport) error {
	...
	err := rttmas_redis.GetRedis().Expire(context.Background(), fmt.Sprintf("user_location_report:%s", ulr.ReporterUID), 30*time.Second)
	if err != nil {
...
func GetBasicInfoByPlate(plate string) map[string]float64 {
	result, err := rttmas_redis.GetRedis().JSONGet(context.Background(), fmt.Sprintf("basic_info:%s", plate), "$").Result()
	if err != nil {
		return nil
	}
