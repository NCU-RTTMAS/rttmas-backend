...
func playbackTimesteps(timesteps []Timestep, geoSearchRadius int) {
	...
	for _, timestep := range timesteps {
		...
		for _, report := range timestep.PlateRecognitionReports {
			...
			rttmas_redis.RedisExecuteLuaScript("geoadd", []string{rkey}, report.Lon, report.Lat, report.PlateNumberSeen)

			rttmas_redis.RedisExecuteLuaScript("adjust_pv_score", []string{"nil"}, timestep.TimeSeconds, report.PlateNumberSeen, report.ReporterUID, report.Lon, report.Lat, geoSearchRadius, 30, 20)
		}
...
func AnalyzeUVBindingAccuracy(xrttmas *xRTTMAS) {
	...
	for i := 0; i < totalUserCount; i++ {
		...
		rawResult, _ := rttmas_redis.RedisExecuteLuaScript("get_most_probable_uid_for_vid", []string{"nil"}, VID)
...
func AnalyzePVBindingAccuracy(xrttmas *xRTTMAS) {
	...
	for _, fact := range xrttmas.PVBindingFacts.VehicleFacts {
		rawResult, _ := rttmas_redis.RedisExecuteLuaScript("get_most_probable_plate_for_vid", []string{"nil"}, fact.VID)
		resultArr := rawResult.([]interface{})
		...
		rawResult, _ = rttmas_redis.RedisExecuteLuaScript("get_vehicle_seen_count", []string{"nil"}, fact.VID)
...
func AnalyzeVIDCreation(xrttmas *xRTTMAS) {
	...
	for _, fact := range xrttmas.PVBindingFacts.VehicleFacts {
		...
		rawResult, _ := rttmas_redis.RedisExecuteLuaScript("analyze_plate_for_uid", []string{"nil"}, UID)
		if rawResult == nil {
			continue
		}
		resultArr := rawResult.([]interface{})
...
func AnalyzeBindingsForDynamicVIDCreation(xrttmas *xRTTMAS) {
	allVIDs, _ := rttmas_redis.RedisExecuteLuaScript("analyze_all_vid_bindings", []string{"nil"})
	if allVIDs == nil {
		logger.Debug("None")
	}
	...
	for _, _vid := range allVIDs.([]interface{}) {
		...
		// Plate Prediction
		rawResult1, _ := rttmas_redis.RedisExecuteLuaScript("get_most_probable_plate_for_vid", []string{"nil"}, VID)
		...
		// UID Prediction
		rawResult2, _ := rttmas_redis.RedisExecuteLuaScript("get_most_probable_uid_for_vid", []string{"nil"}, VID)
...
func AnalysisExperiment() {
	...
	if !analysisOnly {
		...
		rttmas_redis.GetRedis().FlushAll(context.TODO())

		rttmas_redis.RedisExecuteLuaScript("create_indices", []string{"nil"})
